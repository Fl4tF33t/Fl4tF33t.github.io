...
namespace Systems {
    ...
    public class GridSystem2D<T> {
        ...
        private GridSystem2D(int width, int height, float cellSize, Vector3 origin,
            ICoordinateConverter iCoordinateConverter, bool debug) {
            this.width = width;
            this.height = height;
        ...
        public static GridSystem2D<T> CreateGrid(GridLayout layout, int width, int height, float cellSize,
        ...
        public IEnumerable<T> GetNeighbours(int x, int y, IEnumerable<GridDirection> directions) {
            foreach (var dir in directions) {
                int nx = x + dir.Vector.x;
                int ny = y + dir.Vector.y;
                if (!IsValid(nx, ny)) continue;

                T neighbour = GetValue(nx, ny);
        ...
        public Vector3 GetWorldPositionCenter(int x, int y) =>
            iCoordinateConverter.GetToWorldCenter(x, y, cellSize, origin);

        public Vector2Int GetXY(Vector3 worldPositon) =>
            iCoordinateConverter.WorldToGrid(worldPositon, cellSize, origin);

        private void CreateWorldText(GameObject parent, string text, Vector3 position, Vector3 direction,
        ...
        private class VerticalConverter : ICoordinateConverter {
            ...
            public Vector3 GetToWorldCenter(int x, int y, float cellSize, Vector3 origin) =>
                origin + new Vector3(x * cellSize + cellSize * 0.5f, y * cellSize + cellSize * 0.5f, 0);
        ...
        private class HorizontalConverter : ICoordinateConverter {
            ...
            public Vector3 GetToWorldCenter(int x, int y, float cellSize, Vector3 origin) =>
                origin + new Vector3(x * cellSize + cellSize * 0.5f, 0, y * cellSize + cellSize * 0.5f);

            public Vector3 GridToWorld(int x, int y, float cellSize, Vector3 origin) =>
                origin + new Vector3(x, 0, y) * cellSize;
