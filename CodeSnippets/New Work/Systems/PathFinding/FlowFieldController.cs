...
namespace Systems {
    public class FlowFieldController : MonoBehaviour {
        ...
        private void InitializeGrid() {
            ...
            if (Layers.ExtraCost < 0 || Layers.Impassible < 0) throw new InvalidOperationException("Layer doesn't exist");
        ...
        private Cell InitializeCell(int x, int y, Collider[] overlapColliders, Vector3 cellHalfExtents) {
            ...
            var size = Physics.OverlapBoxNonAlloc(cellPos, cellHalfExtents, overlapColliders, Quaternion.identity, costFieldLayers);
        ...
        // TODO: there is a better way for the calculation, use Dijkstra
        private void CreateIntegrationField(Cell destination) {
            ...
            while (cellsToCheck.Count > 0) {
                ...
                var neighbours = FlowField.GetNeighbours(currentCell.x, currentCell.y, GridDirection.CardinalDirections);
                foreach (var neighbourCell in neighbours) {
                    if (neighbourCell.cost == byte.MaxValue) continue;
            
                    var newBestCost = currentCell.bestCost + neighbourCell.cost;
                    if (neighbourCell.bestCost > newBestCost) {
                        neighbourCell.bestCost = (ushort)(newBestCost);
        ...
        private void CreateFlowField() {
            foreach (var cell in FlowField.Grid) {
                ...
                var curNeighbours = FlowField.GetNeighbours(cell.x, cell.y, GridDirection.AllDirections);
                var lowestCost = cell.bestCost;

                foreach (var neighbour in curNeighbours) {
                    ...
                    if (neighbour.bestCost < lowestCost) {
                        ...
                        cell.bestDirection = GridDirection.FromVector(dir);
                    }
                }
            }
        }
        ...
        private void OnDrawGizmos()
        {
            ...
            foreach (var cell in FlowField.Grid)
            {
                Vector3 center = FlowField.GetWorldPositionCenter(cell.x, cell.y) + Vector3.up * 0.01f; // slight Y offset
                ...
                // 2️⃣ Color based on cost (optional)
